...
function getDaysInRange(startIso: string, endIso: string, limit = 7): string[] {
  const days: string[] = [];
  const cur = parseISODateLocal(startIso);
  const end = parseISODateLocal(endIso);
  while (cur <= end && days.length < limit) {
    days.push(
...
function formatRangeLabel(startIso: string, endIso: string): string {
  const fmt = (iso: string) =>
    parseISODateLocal(iso).toLocaleDateString(undefined, { month: 'short', day: 'numeric' });
...
type SlotGridProps = {
  ...
  maxVotes: number;
  /** Show voter dot rows under each column (expanded view only) */
  showVoterDots?: boolean;
  senders?: Record<SenderId, { initial: string; color: string }>;
};

function SlotGrid({ days, today, votesByDay, maxVotes, showVoterDots, senders }: SlotGridProps) {
  return (
    <View style={gridStyles.wrapper}>
      ...
      <View style={gridStyles.row}>
        ...
        {days.map(iso => (
          <View key={`dow-${iso}`} style={gridStyles.dayCol}>
      ...
      <View style={[gridStyles.row, gridStyles.dateRow]}>
        ...
        {days.map(iso => {
          ...
          return (
            <View key={`num-${iso}`} style={gridStyles.dayCol}>
      ...
      {SLOTS.map(slot => (
        <View key={slot} style={gridStyles.row}>
          ...
          {days.map(iso => {
            ...
            return (
              <View key={`${slot}-${iso}`} style={gridStyles.dayCol}>
      ...
      {showVoterDots && senders && (
        <View style={[gridStyles.row, gridStyles.voterRow]}>
          ...
          {days.map(iso => {
            ...
            return (
              <View key={`voters-${iso}`} style={[gridStyles.dayCol, gridStyles.voterDotCol]}>
                {voters.map(s => (
                  <View
                    key={s}
    ...
    <View style={expandedStyles.container}>
      ...
      <View style={expandedStyles.header}>
        <View style={expandedStyles.headerText}>
          {title ? <Text style={expandedStyles.title}>{title}</Text> : null}
          <Text style={expandedStyles.subtitle}>{rangeLabel} · pick your times</Text>
        </View>
...
export function WeekCard({ title, startIso, endIso, votes, senders }: Props) {
  const [expanded, setExpanded] = useState(false);
  const today = useMemo(() => todayIso(), []);

  const days = useMemo(() => getDaysInRange(startIso, endIso), [startIso, endIso]);
  const rangeLabel = useMemo(() => formatRangeLabel(startIso, endIso), [startIso, endIso]);
      ...
      <Pressable onPress={() => setExpanded(true)} style={cardStyles.card}>
        ...
        <View style={cardStyles.footer}>
          <View style={cardStyles.footerLeft}>
            ...
            <Text style={cardStyles.footerVoted}>
              {votedSenderIds.length} voted
            </Text>
          </View>
